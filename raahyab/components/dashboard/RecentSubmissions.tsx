  ...
  return (
    <div className="rounded-3xl border border-foreground/10 my-6 bg-card overflow-hidden">
      <div className="px-8 sm:px-6 py-4 border-b mt-2.5 border-foreground/10">
        <h3 className="text-base font-medium text-foreground">Recent Submissions</h3>
      </div>
      ...
      <div className={`grid ${GRID_COLS} gap-3 px-5 sm:px-6 py-2.5 bg-foreground/1.5`}>
        ...
        <span className="text-[11px] uppercase tracking-wide hidden lg:block pl-4 text-muted-foreground">Deadline</span>
        <span className="text-[11px] hidden lg:block uppercase tracking-wide text-muted-foreground text-right">
          Submitted
        </span>
      </div>
      ...
      <div className="divide-y divide-foreground/8">
        {data.map((opp) => (
          <div
            key={opp.id}
            className={`grid ${GRID_COLS} gap-3 items-center px-5 sm:px-6 py-3.5 border-l-[3px] border-l-primary hover:bg-primary/4 transition-colors`}
          >
            ...
            <div className="text-xs text-foreground pl-4">{formatDeadline(opp.deadline)}</div>
